...
public abstract class MyBaseInterceptor extends AGenericInterceptor {
    ...
    public MyBaseInterceptor() {
        super();
        initialize();
        getLogger().info(String.format("Initialized plugin class %s version %s build date %s support url %s",getClass().getCanonicalName(),MetaData.VERSION, MetaData.BUILDTIMESTAMP, MetaData.SUPPORT_URL));
